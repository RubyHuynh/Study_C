...
void* thread_func() {
    ...
    if ((ret = pthread_mutex_lock(&mutex)) == 0) {
        /* try_lock will not affect locked_context, if unlocked==> lock as normal */
        if ( (ret = pthread_mutex_trylock(&mutex)) == 0 ) {
            ...
            printf("\t errno thread=%d errno=%d(%s) ret=%d(%s)\n", (int) pthread_self(), errno, strerror(errno), ret, strerror(ret));
            /*
errno thread=-304421120 errno=0(Success) ret=16(Device or resource busy)
            */
        }
        printf("\t\t end thread\n");
        pthread_mutex_unlock(&mutex);
